# Spec: Routing Target Types

## Identity

- **Owner agent:** `tech-lead`
- **Consumers:** `/add-specialist`, `/plan-implementation`, `/audit-routing-table`,
  any caller that reads Tech Lead Phase 1 output
- **Memory file:** `.claude/agent-memory/engineering-leaders-tech-lead/MEMORY.md`

## Purpose

Define the five target types supported by the Tech Lead's routing table and
the per-type behavior at registration time, routing time, and Phase 1 output
time. This spec extends `tech-lead-routing` with the target-type model; it
does not replace the base spec.

## Target Type Vocabulary

The five supported target types are:

- `subagent`: a local Claude Code sub-agent resolved via
  `agents/<agent-name>.md` or the path declared on the entry. Dispatched by
  the caller via the Agent tool with a kebab-case slug. **This is the default
  when no target type is declared.**
- `skill`: a skill whose invocation produces the answer. The path-or-slug
  token holds the skill slug. Dispatched by the caller invoking the named
  skill with the emitted prompt as input.
- `doc`: a file path the user should read before the plan proceeds. The
  path-or-slug token holds the file path. Not dispatched; the plan notes the
  dependency.
- `human`: a named person or role whose judgment is required. The path-or-slug
  token holds a contact identifier (name, role, or email). Not dispatched;
  the plan is paused with an explicit escalation notice.
- `external-agent`: a sub-agent in another installed plugin, referenced by a
  namespaced slug (for example, `plugin-x:agent-y`). Dispatched by the caller
  via the Agent tool with the namespaced slug.

Any declared type not in this five-string vocabulary is treated as `subagent`
with a routing warning emitted under `## Preliminary Constraints`.

## Memory File Entry Format

The `## Registered Specialists` section stores one entry per specialist.

**Legacy format (subagent, omitted type):**

```markdown
- `<agent-name>` — `<path-to-agent-file>`
```

**Extended format (non-subagent types):**

```markdown
- `<agent-name>` — `<path-or-slug>` — `target-type: <type>`
```

The `target-type` suffix MUST be written for all non-`subagent` entries.
`subagent` entries MUST use the legacy two-token format to preserve round-trip
compatibility with existing memory files.

## Phase 1 Output Per-Type Anchors

Each `### <Name>` subsection under `## Consultation Requests` MUST include the
following lines in order:

1. `**Target Type:** <type>`, immediately after the subsection heading.
...
3. `**Prompt:**` blockquote (unchanged across all types).

The `**Agent:**`, `**Prompt:**`, `## Consultation Requests`, and `## Next Step`
anchors are preserved unchanged. The new `**Target Type:**` line and
`**Skill:**`, `**Doc:**`, `**Contact:**` anchors are additive. Existing parsers
that match only `**Agent:**` and `**Prompt:**` continue to work; they naturally
treat non-`subagent` requests (which carry no `**Agent:**` line) as
non-spawnable and surface them to the user.

## Caller-Side Dispatch Patterns

See the "Routing Target Types" section of the top-level `README.md` for the
canonical per-type dispatch patterns. In brief:

- `subagent`: spawn via the Agent tool with the local kebab-case slug.
- `external-agent`: spawn via the Agent tool with the namespaced slug.
- `skill`: invoke the skill with the emitted prompt; feed its output into
  Phase 2 like any specialist response.
- `doc`: cite the file path in the plan's dependencies; the user reads the
  file before starting; no Phase 2 feedback loop.
- `human`: pause the plan with an escalation notice naming the contact;
  the user owns the handoff; no automated Phase 2 feedback loop.

`/plan-implementation` dispatch for non-`subagent` target types is a
deliberate follow-up change. The current skill surfaces non-`subagent`
requests to the user rather than dispatching them.

## Registration via `/add-specialist`

The `/add-specialist` skill accepts an explicit target type:

- `--target-type=<type>` flag (unambiguous regardless of argument order), or
- a second positional token matching the five-string vocabulary.

When no target type is provided, the skill registers a `subagent` entry in
the legacy format. Per-type validation rules (applied as warnings, not errors):

- `skill`: warn if `skills/<slug>/SKILL.md` is not present locally.
- `doc`: warn if the path does not resolve via Glob.
- `human`: no format check.
- `external-agent`: warn if the slug does not contain a `:` namespace separator.
- `subagent`: existing behavior (warn if `agents/<name>.md` is missing).

## Invariants

- Entries without a declared target type are treated as `subagent`. The
  absence of a type suffix is semantically equivalent to `target-type: subagent`.
- `subagent` entries are always written in the legacy two-token format. The
  `target-type` suffix is never written for `subagent` entries.
- The `## Routing Outcomes` section is not affected by target-type registration
  or routing. `/add-specialist` and routing procedures must leave it untouched.
- Unknown target types produce a routing warning and fall back to `subagent`.
  They are never silently dropped.

## Acceptance Criteria

- [ ] A `skill` entry registered via `/add-specialist` produces
      `**Target Type:** skill` and `**Skill:** <slug>` in Phase 1 output when
      the issue matches.
- [ ] A `doc` entry registered via `/add-specialist` produces
      `**Target Type:** doc` and `` **Doc:** `<path>` `` in Phase 1 output.
- [ ] A `human` entry registered via `/add-specialist` produces
      `**Target Type:** human` and `**Contact:** <name-or-role>` in Phase 1
      output.
- [ ] An `external-agent` entry registered via `/add-specialist` produces
      `**Target Type:** external-agent` and `**Agent:** <plugin:agent>` in
      Phase 1 output.
- [ ] A legacy entry (no target-type suffix) produces
      `**Target Type:** subagent` in Phase 1 output and continues to route
      correctly.
- [ ] `/plan-implementation` is not modified by this change. The
      `skills/plan-implementation/SKILL.md` file is unchanged.
