# Spec: Convention Domain Ownership

## Identity

- **Owner agent:** `tech-lead` (as cross-domain registrar)
- **Consumers:** `/write-convention`, Convention Authorship subsections in
  `agents/devops-lead.md`, `agents/qa-lead.md`, `agents/ux-strategist.md`,
  `agents/chief-architect.md`, `agents/tech-lead.md`
- **Memory file:** `.claude/agent-memory/engineering-leaders-tech-lead/MEMORY.md`

## Purpose

Define a convention domain ownership model that assigns each convention domain
to a single declared owner agent. The model enables projects to grow convention
coverage in multiple domains in parallel without routing all authorship through
the Tech Lead. The Tech Lead retains the cross-domain registrar role and
remains the sole author for `tactical-implementation` conventions.

## Domain Vocabulary

The plugin recognizes five convention domains, each mapped to a single owner
agent:
...
The domain vocabulary is fixed. Unknown domains are rejected by the
`/write-convention` skill with a list of valid values.

## Default Values and Back-Compat

A missing `domain` field resolves to `tactical-implementation` at read time.
A missing `owner` field resolves to `tech-lead` at read time. These defaults
preserve back-compat with existing projects that created conventions before
this change.

Memory-file writers and the `/write-convention` skill MUST NOT back-fill
defaults into legacy entries that already round-trip cleanly. Writes for new
`tactical-implementation` entries owned by `tech-lead` MAY omit the domain
and owner suffixes to preserve the legacy row format. Writes for any other
domain or owner MUST include both fields explicitly.

## Index Entry Format

The conventions index supports per-entry `domain` and `owner` metadata using
a trailing suffix shape:

```markdown
- `<convention-name>` — `<path-to-doc>` — `domain: <domain>` — `owner: <agent>`
```

Entries that omit the suffixes are read as `tactical-implementation` owned by
`tech-lead`. The index format is human-editable; readers MUST tolerate
additional whitespace and MUST accept both `—` (em dash) and `-` (hyphen) as
the separator between name, path, and suffixes.

## Convention Document Frontmatter

Every convention document authored via `/write-convention` carries a YAML
frontmatter block with at minimum:
...
- `status`: initially `draft`; promoted to `active` only after human review,
  consistent with the existing "drafts until merged" rule

Convention documents without frontmatter are read as
`domain: tactical-implementation` and `owner: tech-lead` at read time,
preserving back-compat with existing conventions. This spec does NOT require
a migration pass over existing convention files.

## Tech Lead Convention Responsibilities

The Tech Lead retains convention responsibility in four reframed ways:

- **Author** for conventions whose domain is `tactical-implementation`. The
  existing Convention Authorship procedure in `agents/tech-lead.md` is scoped
  to that domain and remains unchanged in substance.
- **Registrar** for the conventions index across all domains. After a domain
  owner drafts and a human reviews a convention, the Tech Lead records the
  entry in the index with domain and owner fields populated.
- **Cross-domain reviewer.** When a draft or an existing convention has
  implications in a second domain, the Tech Lead surfaces the cross-domain
  dependency and routes the consultation to the relevant owner.
- **Gap identifier.** When an inconsistency surfaces (during code review,
  postmortem, or retrospective), the Tech Lead identifies the domain the gap
  belongs in and points at the owner whose Convention Authorship subsection
  should consider a draft.

The Tech Lead MUST NOT author convention drafts in domains other than
`tactical-implementation`. When a user asks the Tech Lead to draft in another
domain, the Tech Lead MUST point at the owner agent and at
`/write-convention --domain=<domain>`.

## `/write-convention` Skill Contract

The skill at `skills/write-convention/SKILL.md` accepts an optional
`--domain=<value>` flag or a positional token matching the five-string
vocabulary. When the argument is omitted, the skill defaults to
`tactical-implementation` owned by `tech-lead`. The skill validates the domain
against the fixed vocabulary and prompts for a valid value when an unknown
domain is supplied.

The skill routes the drafting procedure to the owner agent declared for the
domain. The drafting flow:

1. Reads the conventions directory path from the Tech Lead's memory.
2. Reads the project's convention template if one is recorded in memory.
3. Produces a draft file in the conventions directory with frontmatter at
   minimum: `name`, `domain`, `owner`, `status: draft`.
4. Emits a confirmation summary showing the draft path, domain, owner, and a
   note that the Tech Lead will handle index registration after review.

The skill MUST NOT write to or update the conventions index. Index registration
is the Tech Lead's responsibility per the Tech Lead Convention Responsibilities
section above.

When the conventions directory path is not recorded in the Tech Lead's memory,
the skill surfaces a clear error naming the missing configuration and pointing
the user at the Tech Lead's onboarding path.

## Invariants

- The domain vocabulary is fixed at five strings. Projects cannot extend it
  unilaterally.
- Each domain has exactly one declared owner agent. Multi-owner conventions
  are represented by a single primary owner plus informal notes in the
  document body.
- The Tech Lead MUST NOT author convention drafts outside the
  `tactical-implementation` domain.
- The `/write-convention` skill MUST NOT register drafts in the conventions
  index; registration is the Tech Lead's responsibility after human review.
- Existing conventions without declared domain or owner default to
  `tactical-implementation` owned by `tech-lead` at read time; no migration
  pass is required or triggered.

## Non-Requirements

- Enforcing ownership at runtime is out of scope; ownership is an
  organizational signal surfaced in drafts, the index, and the README, not a
  gate.
- Migrating existing conventions to new owners is out of scope; migration is
  case-by-case.
- Multi-owner conventions with formal multi-domain semantics are deferred.
- A domain extension mechanism for project-specific domains is deferred until
  feedback warrants it.
- An audit check for conventions whose declared owner does not match the domain
  mapping is deferred to a later change.

## Acceptance Criteria

- [ ] Each of the five domains is mapped to exactly one owner agent in the
      README Convention Ownership Matrix, this spec, and the `/write-convention`
      skill routing table, with no contradictions.
- [ ] `/write-convention --domain=infrastructure <name>` routes to the DevOps
      Lead's Convention Authorship procedure and produces a draft with
      `domain: infrastructure` and `owner: devops-lead` in frontmatter.
- [ ] `/write-convention --domain=garbage <name>` emits a message listing the
      five valid domains and prompts for a valid value without producing a draft.
- [ ] `/write-convention <name>` with no domain argument defaults to
      `domain: tactical-implementation` and `owner: tech-lead`.
- [ ] The Tech Lead redirects a non-`tactical-implementation` authorship
      request to the declared domain owner without producing a draft itself.
- [ ] Legacy index entries (no domain or owner suffix) are read as
      `tactical-implementation` owned by `tech-lead` without warning.
- [ ] `agents/devops-lead.md`, `agents/qa-lead.md`, `agents/ux-strategist.md`,
      and `agents/chief-architect.md` each contain a Convention Authorship
      subsection naming the agent's declared domain and documenting the
      drafting procedure.
- [ ] `skills/write-adr/SKILL.md`, `skills/write-runbook/SKILL.md`,
      `skills/plan-implementation/SKILL.md`, and
      `skills/refinement-review/SKILL.md` are unchanged by this change.
