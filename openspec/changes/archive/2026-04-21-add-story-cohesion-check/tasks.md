...
- [x] 2.1 Add a "Principle 8 — Single-Concept Cohesion" subsection to the
      Evaluate Coaching Principles section of `skills/refine-story/SKILL.md`
      with the six failure signals enumerated as bullets
- [x] 2.2 Document the two-signal quorum rule: FAIL when two or more
      ...
      are observed
- [x] 2.3 Add a Suggested split output block specification under the
      principle's description, showing the per-child role / capability /
      benefit format and AC mapping
- [x] 2.4 Extend the Output section's principle enumeration to include
      principle #8 with the "Suggested split" action block shape (not a
      "Suggested rewrite")
- [x] 2.5 Update the Summary line in the Output section from "Y of 7
      coaching principles passing" to "Y of 8 coaching principles passing"
- [x] 2.6 Confirm that principles #1 through #7 are unchanged in
      wording, failure signals, and suggested-rewrite shapes
- [x] 2.7 Confirm that the INVEST Scorecard, Definition of Done, and
      Horizontal Work sections are structurally unchanged

## 3. Update the agile-coach agent definition

- [x] 3.1 Add an intentional-vs-accidental judgment overlay to the Key
...
- [x] 3.2 Document the atomicity test: ask whether the bundled items must
      ship together to avoid a broken intermediate state; when yes the
      bundle is intentional and the FAIL is a false positive
- [x] 3.3 Document the downstream action for each case: accidental bundle
...
- [x] 3.4 Update the Story Review subsection of How to Respond so the
      coach explicitly applies the overlay each time `/refine-story`
      returns a Single-Concept Cohesion FAIL
- [x] 3.5 Confirm the agent's Jurisdiction section is updated to include
...
- [x] 4.5 Confirm the agent's Delegation section notes that post-coaching
      scope review handles cohesion findings the same way it handles
      other scope findings

## 5. Add a documentation example

- [x] 5.1 Add an example to the plugin README, the `refine-story` skill,
...
- [x] 5.2 The example MUST include the original conflated draft, the
      Suggested split block output, and both child stories as they would
      be authored independently
- [x] 5.3 The example MUST show the intentional-bundle alternative (a
...
- [x] 6.1 Confirm that `skills/write-story/SKILL.md` is not modified by
      this change
- [x] 6.2 Confirm that `skills/write-epic/SKILL.md`,
      `skills/write-bug/SKILL.md`, `skills/write-spike/SKILL.md`, and
      `skills/decompose-requirement/SKILL.md` are not modified by this
      change
- [x] 6.3 Confirm that the INVEST Scorecard and the seven existing
      coaching principles are unchanged in wording and failure signals
- [x] 6.4 Confirm that the Agile Coach's existing overlays (horizontal-
      work legitimacy, scope-boundary norms, DoD placement) are unchanged
      and the new cohesion overlay is additive
- [x] 6.5 Confirm that the Product Owner's existing Requirement Authoring
...
- [x] 7.1 Invoke `/refine-story` with a conflated draft (two unrelated
...
- [x] 7.5 Consult the Agile Coach inline during `/write-story` with a
...
- [x] 7.6 Consult the Product Owner with a user rejection of the split
...
- [x] 7.7 Confirm the `/refine-story` Summary line reads "X of 6 INVEST
