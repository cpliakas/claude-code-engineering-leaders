...
- [ ] 1.1 Add a top-level section to `README.md` titled "Tech Lead Orchestration
      Tiers" positioned near the existing "Plan Implementation for a Refined
      Story with the Tech Lead" example
- [ ] 1.2 Document all three tiers by name (`1 — Direct specialist`,
      `2 — Standard`, `3 — Full (with Architect escalation)`) with one
      paragraph of framing per tier and a one-sentence "when to use it"
- [ ] 1.3 Include at least two concrete examples per tier drawn from realistic
      engineering work
- [ ] 1.4 Cross-reference the existing Tech Lead example and the
...
- [ ] 2.1 Add a "Signals Catalog" subsection inside the new orchestration-
      ...
      area heuristic
- [ ] 2.2 Include an "If in doubt, escalate" callout with a one-paragraph
      explanation of why defaulting downward is the failure mode the catalog
      prevents
- [ ] 2.3 Cross-reference the Chief Architect's description triggers (one-way
...
- [ ] 3.5 Update the Parseable Phase 1 Output Contract subsection to document
      the new `## Engagement Tier` anchor and note that parsers MAY ignore
      it (backward compatibility)
- [ ] 3.6 Update the Phase 2 synthesis procedure so the Escalation Flags
...
- [ ] 3.7 Cross-reference the README signals catalog from the agent so the
      two documents stay aligned
- [ ] 3.8 Add an explicit user-override note: when the caller states a tier
...
- [ ] 4.1 Read `skills/plan-implementation/SKILL.md` and confirm its Phase 1
...
- [ ] 4.2 Confirm the new `## Engagement Tier` line does not fall between
      any of those anchors in a way that would confuse parsing
- [ ] 4.3 Document in the agent's Parseable Phase 1 Output Contract that
      `## Engagement Tier` is a non-breaking addition

## 5. Internal consistency review

- [ ] 5.1 Re-read the updated `agents/tech-lead.md` and confirm Rule 4's
      wording still reads as a strong invariant within its scoped tiers
- [ ] 5.2 Re-read the updated `README.md` and confirm the signals catalog
      is concrete (named vocabulary, numeric bands) rather than platitudinous
- [ ] 5.3 Confirm no text changes were made to `agents/chief-architect.md`
- [ ] 5.4 Confirm no text changes were made to
      `skills/plan-implementation/SKILL.md`
- [ ] 5.5 Confirm no text changes were made to
      `skills/refinement-review/SKILL.md`

## 6. Manual verification

- [ ] 6.1 Dry-run the Tech Lead on a single-file rename story and confirm
      tier 1 is identified, one specialist is named, and no full routing
      pass occurs
- [ ] 6.2 Dry-run the Tech Lead on a three-file change within one domain
      and confirm tier 2 is identified and the standard two-phase protocol
      runs
- [ ] 6.3 Dry-run the Tech Lead on a cross-cutting story containing
...
- [ ] 6.4 Dry-run the Tech Lead with an explicit user-stated tier and
      confirm the override wins and is recorded in the Engagement Depth
      rationale
- [ ] 6.5 Dry-run `/plan-implementation` on a representative story and
